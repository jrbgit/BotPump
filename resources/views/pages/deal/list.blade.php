    ...
    <tbody>
      @foreach($active_deals_list as $active)
        <tr>
          ...
          <td>{{ $active->status }}</td>
          <td>{{ $active->take_profit }}%</td>
          <td>{{ $active->bought_amount }}</td>
          <td>{{ $active->bought_volume }}</td>
      ...
      @foreach($all_completed_deals as $completed)
        <tr>
          <td>
            ...
            <a href="{{ route('basic.bot.show', $active->bot_id ) }}" class="label bg-primary" title="Show Bot">{{ $active->bot_name }}</a>
          </td>
          ...
          <td>
            @if ($completed->status == 'panic_sold')
            ...
            @endif
          {{ str_replace('_', ' ', $completed->status) }}
          </span></td>
          <td>{{ $completed->final_profit_percentage }}%</td>
